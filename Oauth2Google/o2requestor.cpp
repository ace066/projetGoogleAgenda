...
void O2Requestor::setO2(O2 *authenticator)
{
    authenticator_=authenticator;
}

void O2Requestor::setmanager(QNetworkAccessManager *manager)
{
    manager_=manager;
}
...
//        QByteArray donne = reply_->readAll();
//        qDebug()<<donne;
//        trace()<<reply_->readAll();
        emit finished(0,QNetworkReply::NoError,reply_->readAll());
    }
}
