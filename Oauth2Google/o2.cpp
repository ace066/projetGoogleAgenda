...
//        qDebug() <<"parametres : "<< params.toString(QUrl::FullyEncoded).toUtf8();
...
//        setCode(response.value(QString(O2_OAUTH2_CODE)));
        QString monCode;
        monCode = response.value(QString(O2_OAUTH2_CODE));
        if (monCode!="") setCode(monCode);
...
//        parameters.insert(O2_OAUTH2_CODE, code());
        if(monCode!="")
        {
            parameters.insert(O2_OAUTH2_CODE, monCode);
        }
        else
        {
            parameters.insert(O2_OAUTH2_CODE, code());
